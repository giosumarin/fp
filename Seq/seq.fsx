...
0               e0    e1   e2    e3   e4     e5    e6   e7   e8    e9
                      ^^^^^^^^^^^^^^^^^^           ^^^^^^^^^^^^^^^^^^
                           sq1                           sq1      
...
let rec rsqTrue =
    seq { yield true
          yield! rsqTrue } ;;
// l'interprete segnala un warning che puo' essere ignorato
...
let rec map f sq =
    ...
    // (Seq.item 0) e  (Seq.skip 1) su sq non falliscono
    seq{ let e0 = Seq.item 0 sq 
         yield f e0
...
let rec filter pred sq =
    seq{
       let e0 = Seq.item 0 sq // e0 e' definito in quanto sq e' infinita
...
Esercizio: sequenza delle somme di una sequenza
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
alla sequenza infinita

  n0 + n1, n2,  n3, n4, ...

Quest'ultima sequenza e' facilmente costruibile a partire da sq.
...
let rec sumSeq sq =
    seq{
        let n0 = Seq.item 0 sq
        yield  n0 // primo elemento
        let n1 = Seq.item 1 sq
...
* Siano LB1 e LB2 due espressioni tali che:
  ...
  Allora
    ...
    LB :  unit -> bool     e' la rappresentazione lazy di ( B1 AND B2 )
...
// rappresentazione lazy delle costanti booelane true e false
...
let lazyAnd lb1  lb2 =
    let b1 = lb1 ()  // estraggo l'espressione in lb1
    match b1 with
    | false -> lazyFalse  
    | _ ->  lb2
// lazyAnd : lb1:(unit -> bool) -> lb2:(unit -> bool) -> (unit -> bool)

// Esempi

let e1 = (fun x ->   5 > 0) :   (unit -> bool)  // rappr. lazy di 5 > 0
let e2 = (fun x ->  10 < 0)  :  (unit -> bool)  // rappr. lazy di 10 < 0
let e3 = (fun x ->  2/0 > 0) :  (unit -> bool)  // rappr. lazy di 2/0 > 0
...
Sia t un termine t di tipo  Lazy<T> e supponiamo che t corrisponda alla
rapppresentazione lazy della espressione Expr  di tipo T
Allora:

   t.Force

restituisce  la funzione costante  di tipo unit ->   il cui cui valore e' Expr 
               
Quindi, per valutare Expr occorre applicare alla funzione il valore ():
...
let t1 =  lazy ( 10 +  2 ) ;;  // t1 e' la rappresentazione lazy dell'espressione 10 +  2
...
Riscrivere la funzione lazyAnd usando  usando la funzione lazy
per la rappresentazione lazy di espressioni booleane.
...
///////////////////////////////////////////////
// soluzione usando lazy

let lazyTrue1  = lazy true     // rappresentazione lazy di true
let lazyFalse1 = lazy false    // rappresentazione lazy di false


let lazyAnd1 (lb1 : Lazy<bool>)  lb2 =
    let b1 = lb1.Force()
    match b1 with
    | false -> lazyFalse1
    | _  -> lb2
// lazyAnd1 : Lazy<bool> -> Lazy<bool> -> Lazy<bool>

let g1 = lazy (5 > 0) ;;     // rappr. lazy di 5 > 0
let g2 = lazy (10 < 0) ;;    // rappr. lazy di 10 < 0
let g3 = lazy (2/0 > 0) ;;   // rappr. lazy di 2/0 > 0

let w1 =  lazyAnd1 g1 g1 ;;  // (5 > 0) && (5 > 0)
w1.Force() ;; // true

let w2 = lazyAnd1 g1 g2 ;;  // (5 > 0) && (10 < 0)
w2.Force () ;;  // false

let w3 =  lazyAnd1 g2 g3 ;;  // (10 < 0) &&  (2/0 > 0)
w3.Force () ;; // false

let w4 = lazyAnd1  g1 ( lazyAnd1 g2 g3 );;  // (5 > 0) && ( (10 < 0) &&  (2/0 > 0))
w4.Force () ;; // false

let w5 =  lazyAnd1 lazyTrue1 ( lazyAnd1 lazyFalse1 g3 );;  // true  && ( false &&  (2/0 > 0))
w5.Force () ;; // false

let w6 =  lazyAnd1 lazyTrue1 g3 ;;  // true && (2/0 > 0)
