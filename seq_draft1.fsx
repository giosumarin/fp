...
Seq.take 10 nat1 |> Seq.toList;;
let nat2 =  Seq.initInfinite (fun x ->if (x=5) then -5 else x);;
let even10 =  Seq.initInfinite  (fun x -> 10+x*2);;
...
Seq.take 50 (intFrom -7) |> Seq.toList;;

let rec intFrom1 n= cons n (intFrom1 (n + 1));;
...
// let rec intFrom1 n = ...
 ...
 let rec intFrom12 n=seq{
    if n <> 5 then yield n
    yield! intFrom12 (n + 1)
 }

 let rec intFrom123 n=seq{
